...
everything. Each walked question must map **1:1 to a change just read in the map** - that pairing is
what makes it judgeable. (Batching low-stakes items into grouped prompts is **not pursued** - tried
and reverted: a grouped prompt strips the per-item context needed to judge canonicality; the
roughly-a-minute-per-question round-trip cost is accepted.) For an **`Agent`-claim confirmation**,
offer **`WIP / not-settled` as a first-class
